...
struct Record
{
   ...
   struct RawData
   {
      [[nodiscard]] static inline RawData create(const void *ptr, size_t sz)
      {
         RawData r;
         r.ptr = ptr;
         r.sz  = sz;
   ...
   struct Separator
   {
      [[nodiscard]] static inline Separator create()
      {
         Separator r;
         return r;
      };
      [[nodiscard]] static inline Separator create(const char *separator, bool once = false)
      {
         Separator r;
         r.separator.appendStringAL(separator);
         r.once = once;
         return r;
      };
      template <size_t N>
      [[nodiscard]] static inline Separator create(const char (&separator)[N], bool once = false)
      {
         Separator r;
         r.separator.appendString(separator);
         r.once = once;
         return r;
      };
   ...
   struct SkipSeparator
   {
      ...
      [[nodiscard]] static inline SkipSeparator create(int count)
      {
         return SkipSeparator(count);
      };
      int count {0};
   };
   ...
   [[nodiscard]] static Record
       create(Severity severity, int line, const char *file, const char *fileOnly, const char *func);
   [[nodiscard]] static Record create(Flags flags);

   Record() = default;
   [[deprecated("When setting PODs after creation, the optimizer will set them directly. See "
                "https://godbolt.org/z/En8Wb3sqe")]] inline Record(uninitialized_tag)
   {
   }

   inline void appendMessage(const char *msg, size_t len)
   {
      handleSeparators(len ? *msg : 0);
      message.appendString(msg, len);
   }
   template <size_t N>
   inline void appendMessage(const char (&msg)[N])
   {
      appendMessage(msg, N - 1);
   }
   inline void appendMessageAL(const char *msg)
   {
   ...
   inline size_t getMessageLen() const
   {
      return message.getStringLen();
   }

   [[nodiscard]] inline auto backupFlags()
   {
      return I::CreateFinally([this, fl = flags]() { flags = fl; });
   }
   inline bool hasFlags(Flags value) const
   {
      return (flags & (int)value) == (int)value;
   }
   inline bool hasFlagsAny(Flags value) const
   {
      return (flags & (int)value);
   }
   inline void flagsOn(Flags value)
   {
      flags |= (int)value;
      combineFlags();
   }
   inline void flagsOff(Flags value)
   {
      flags &= ~(int)value;
   }

   template <typename... Ts>
   inline bool hasFlags(Ts... values) const
   {
      return (flags & I::combineInt(values...)) == I::combineInt(values...);
   }
   template <typename... Ts>
   inline bool hasFlagsAny(Ts... values) const
   {
      return (flags & I::combineInt(values...));
   }
   template <typename... Ts>
   inline void flagsOn(Ts... values)
   {
      flags |= I::combineInt(values...);
      combineFlags();
   }
   template <typename... Ts>
   inline void flagsOff(Ts... values)
   {
      flags &= ~I::combineInt(values...);
   }

#ifdef ALOG_ENABLE_DEBUG
   [[nodiscard]] inline auto verifySkipSeparators(int delta = 0)
   {
      return I::CreateFinally([this, ss = (I::max)(skipSeparators + delta, 0)]() { assert(ss == skipSeparators); });
   }
#   define VERIFY_SKIP_SEPARATORS(record, num) auto _checkSS = record.verifySkipSeparators(num);
#else
   [[nodiscard]] inline Nothing verifySkipSeparators(int = 0)
   {
      return {};
   }
#   define VERIFY_SKIP_SEPARATORS(record, num)
   ...
   [[nodiscard]] inline auto updateSkipSeparators(int value)
   {
      auto r = verifySkipSeparators();
      skipSeparators += value;
      return r;
   }
   inline void updateSkipSeparatorsCF(int value)
   {
      skipSeparators += value;
   }
   ...
   Record &&seps()
   {
      flagsOn(Flags::Separators);
      return std::move(*this);
   }
   template <size_t N>
   Record &&seps(const char (&value)[N])
   {
      flagsOn(Flags::Separators);
      separator.clear();
      separator.appendString(value);
      return std::move(*this);
   }
   Record &&seps(const char *value)
   {
      flagsOn(Flags::Separators);
      separator.clear();
      separator.appendStringAL(value);
      return std::move(*this);
   }
   Record &&no_seps()
   {
      flagsOn(Flags::NoSeparators);
      return std::move(*this);
   }
   Record &&quotes()
   {
      flagsOn(Flags::AutoQuote);
      return std::move(*this);
   }
   Record &&no_quotes()
   {
      flagsOn(Flags::NoAutoQuote);
      return std::move(*this);
   }

   // first member variable must be unchecked type to avoid false positive in static code checker
   ...
   Severity severity {Severity::Minimal};
   int line {};
 ...
 private:
   inline void handleSeparators(char /*nextSymbol*/)
   {
      ...
      if (!hasFlags(Flags::Separators) || hasFlags(Flags::Internal_NoSeparators) || !separator || !message)
         return;
   ...
   {
      const bool quoteLiterals = hasFlagsAny(Flags::QuoteLiterals, Flags::Internal_QuoteLiterals);
      const bool autoQuote     = hasFlags(Flags::AutoQuote);

      if (!quoteLiterals && (literal || !autoQuote))
         return;
   ...
   {
      if (!hasFlags(Flags::Internal_QuoteClose))
         return;
...
template <
    typename T,
    typename std::enable_if_t<std::is_integral<T>::value && !std::is_signed<T>::value && sizeof(T) == 1> * = nullptr>
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (uint8_t)value);
}

template <
    typename T,
    typename std::enable_if_t<std::is_integral<T>::value && !std::is_signed<T>::value && sizeof(T) == 2> * = nullptr>
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (uint16_t)value);
}

template <
    typename T,
    typename std::enable_if_t<std::is_integral<T>::value && !std::is_signed<T>::value && sizeof(T) == 4> * = nullptr>
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (uint32_t)value);
}

template <
    typename T,
    typename std::enable_if_t<std::is_integral<T>::value && !std::is_signed<T>::value && sizeof(T) == 8> * = nullptr>
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (uint64_t)value);
}
...
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (int8_t)value);
}
...
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (int16_t)value);
}
...
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (int32_t)value);
}
...
inline ALog::Record &&operator<<(ALog::Record &&record, T value)
{
   return (std::move(record) << (int64_t)value);
}
...
template <typename T>
inline typename std::enable_if_t<std::is_same<T, QString>::value, ALog::Record> &&operator<<(ALog::Record &&record,
                                                                                             const T &value)
{
...
template <typename T>
inline typename std::enable_if_t<std::is_same<T, QLatin1String>::value, ALog::Record> &&
    operator<<(ALog::Record &&record, const T &value)
{
   ALog::onStringQuote1(record);
   record.appendMessage(value.data(), value.size());
   ALog::onStringQuote2(record);
...
template <typename T>
inline typename std::enable_if_t<std::is_same<T, QStringRef>::value, ALog::Record> &&operator<<(ALog::Record &&record,
                                                                                                const T &value)
{
...
template <typename T>
inline typename std::enable_if_t<std::is_same<T, QByteArray>::value, ALog::Record> &&operator<<(ALog::Record &&record,
                                                                                                const T &value)
{
...
template <typename T>
inline typename std::enable_if_t<std::is_same<T, QPoint>::value, ALog::Record> &&operator<<(ALog::Record &&record,
                                                                                            const T &value)
{
...
template <typename T>
inline typename std::enable_if_t<ALog::I::is_container<T>::value && ALog::I::has_key<T>::value &&
                                     ALog::I::is_qt_container<T>::value,
                                 ALog::Record> &&
    operator<<(ALog::Record &&record, const T &value);
...
template <typename T>
inline typename std::enable_if_t<ALog::I::is_container<T>::value && !ALog::I::has_key<T>::value, ALog::Record> &&
    operator<<(ALog::Record &&record, const T &value);

template <typename T>
inline typename std::enable_if_t<std::is_array<T>::value, ALog::Record> &&operator<<(ALog::Record &&record,
                                                                                     const T &value);
// -----
